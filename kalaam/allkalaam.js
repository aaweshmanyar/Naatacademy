async function statusloading() {
  try {
    ...
    if (allTab)
      allTab.innerHTML = `تمام (${
        stats.poetry + stats.articles + stats.writers || 0
      })`;
    ...
    if (container)
      container.innerHTML =
        "<p class='text-red-600'>ڈیٹا لوڈ کرنے میں مسئلہ ہوا۔</p>";
  }
}
...
async function loadTabData() {
  try {
    const [kalaamRes, articleRes, writerRes] = await Promise.all([
      fetch(
        "https://updated-naatacademy.onrender.com/api/kalaam/limited?limit=4&offset=16"
      ),
    ...
    kalaamData?.data?.forEach((kalaam) => {
      // Extract first 2 lines of ContentUrdu per kalaam
      const lines = (kalaam.ContentUrdu || "").split("\n").slice(0, 2);
      const excerpt = lines.join("<br>") || "اقتباس دستیاب نہیں";
      ...
      allContainer.innerHTML += `
            <article class="result-item-card">
                ...
                <div class="item-meta">
                    ...
                    <div class="meta-stats">
                        <span class="urdu-text-xs flex items-center gap-1">
                            <i class="bi bi-heart-fill text-red-400"></i> ${article.likes || 130}
                        </span>
                        <span class="urdu-text-xs flex items-center gap-1">
                            <i class="bi bi-eye-fill text-blue-400"></i> ${article.views || 200}
                        </span>
                    </div>
                    <a href="/articles/${article.ArticleID}" class="read-more-link urdu-text">
                      <span>پڑھیں</span><i class="bi bi-arrow-left-short"></i>
                    </a>
                </div>
            </article>
    ...
    writerData?.data?.forEach((writer) => {
      allContainer.innerHTML += `
            <article class="result-item-card writer-card">
                ...
                <a href="/writers/${writer.WriterID}" class="read-more-link urdu-text">
  ...
  try {
    while (moreData) {
      const response = await fetch(
        `https://updated-naatacademy.onrender.com/api/kalaam/limited?limit=${limit}&offset=${offset}`
      );
      ...
      kalaams.forEach((kalaam) => {
        // Split ContentUrdu by line break, take first 2 lines, join with <br> if both lines exist
        const lines = (kalaam.ContentUrdu || "").split("\n").slice(0, 2);
        const excerpt = lines.join("<br>") || "اقتباس دستیاب نہیں";

        container.innerHTML += `
                    <article class="result-item-card">
                        <h2 class="kalam-title urdu-text urdu-text-md">${
                          kalaam.Title
                        }</h2>
                        <p class="kalam-excerpt urdu-text urdu-text-base line-clamp-2">${excerpt}</p>
                        <div class="item-meta">
                            <div class="flex items-center gap-2">
                                ...
                                <p class="urdu-text urdu-text-xs m-0 p-0">${
                                  kalaam.WriterName || "نامعلوم شاعر"
                                }</p>
                            </div>
                            <div class="meta-stats">
                                <span class="urdu-text-xs flex items-center gap-1">
                            ...
                            <a href="../lyrics.html?id=${
                              kalaam.KalaamID
                            }" class="read-more-link urdu-text">
                                <span>پڑھیں</span><i class="bi bi-arrow-left-short"></i>
                            </a>
                        </div>
                    </article>
    ...
    container.innerHTML =
      "<p class='text-red-500 urdu-text'>کلام لوڈ کرنے میں خرابی پیش آئی۔</p>";
  }
}
...
async function loadAllArticles() {
  ...
  try {
    ...
    if (result.length === 0) {
      container.innerHTML =
        "<p class='urdu-text text-gray-500'>کوئی مضمون دستیاب نہیں ہے۔</p>";
      return;
    }
    ...
    result.forEach((article) => {
      container.innerHTML += `
                <article class="result-item-card">
                    <h2 class="article-title urdu-text urdu-text-md">${
                      article.Title
                    }</h2>
                    
                    <div class="item-meta">
                        <div class="flex items-center gap-2">
                            <p class="urdu-text urdu-text-xs m-0 p-0">مضمون نگار: ${
                              article.WriterName || "نامعلوم"
                            }</p>
                        </div>
                        <div class="meta-stats">
                            <span class="urdu-text-xs flex items-center gap-1">
    ...
    container.innerHTML =
      "<p class='text-red-500 urdu-text'>مضامین لوڈ کرنے میں خرابی پیش آئی۔</p>";
  }
}
...
async function loadAllWriters() {
  ...
  try {
    ...
    if (result.length === 0) {
      container.innerHTML =
        "<p class='urdu-text text-gray-500'>کوئی شاعر دستیاب نہیں ہے۔</p>";
      return;
    }
    ...
    result.forEach((writer) => {
      ...
      container.innerHTML += `
                <article class="result-item-card writer-card">
                    ...
                    <div class="flex-grow">
                        <h2 class="writer-name urdu-text urdu-text-md">${
                          writer.Name || writer.Name
                        }</h2>
                        <p class="writer-meta urdu-text urdu-text-xs">${
                          writer.Bio || "تفصیل دستیاب نہیں"
                        }</p>
                    </div>
                    <a href="../poet.html?id=${
                      writer.WriterID
    ...
    container.innerHTML =
      "<p class='text-red-500 urdu-text'>شاعر لوڈ کرنے میں خرابی پیش آئی۔</p>";
  }
}
