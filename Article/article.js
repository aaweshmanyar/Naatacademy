...
async function loadArticleDetail() {
    ...
    try {
      const res = await fetch(`https://api.naatacademy.com/api/articles/${articleId}`);
      if (!res.ok) throw new Error("Failed to fetch article");
      ...
      // Handle article header image - hide if not available
      const articleHeaderImage = document.querySelector(".article-header-image");
      if (article.imageUrl) {
        articleHeaderImage.src = article.imageUrl;
        articleHeaderImage.style.display = "block"; // Make sure it's visible
      } else {
        articleHeaderImage.style.display = "none"; // Hide the image element
      }
...
async function RelatedArticle() {
  try {
    const res = await fetch(`https://api.naatacademy.com/api/articles`);
    if (!res.ok) throw new Error("Failed to fetch articles");
    ...
    articles.slice(0, 4).forEach((article) => {
      ...
      const imageHtml = article.imageUrl 
        ? `<img src="${article.imageUrl}" alt="${article.Title}" class="card-thumbnail">`
        : '';
